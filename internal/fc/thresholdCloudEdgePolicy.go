...
func (p *ThresholdCloudEdgePolicy) OnCompletion(_ *scheduledFcRequest) {
}

func (p *ThresholdCloudEdgePolicy) OnArrival(r *scheduledFcRequest) {
	...
	if r.CanDoFcOffloading && !r.IsInProfilingMode && r.Iteration == 0 {
		for _, fun := range r.Fc.Functions {
			...
			if !created {
				fmt.Println("There are not warm containes for function: ", fun)
