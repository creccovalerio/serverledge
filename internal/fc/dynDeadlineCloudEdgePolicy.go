...
var currentDataDdcep ReturnedQueryMetrics
...
func (p *DynDeadlineCloudEdgePolicy) SubmitInfos(data ReturnedQueryMetrics) {
	timestamp := time.Now()
...
func (p *DynDeadlineCloudEdgePolicy) OnArrival(r *scheduledFcRequest) {
	...
	if r.CanDoFcOffloading && !r.IsInProfilingMode &&
		(r.QoSMaxFcRespT > 0 && r.QoSMaxFcRespT <= currentDataDdcep.AvgFcRespTime[r.Fc.Name]) {
